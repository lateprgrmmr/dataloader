...
def SplitNames(file, cols):
    ...
    try:
        ...
        for row in df[cols]:
            # -- Removes current known portions of unformatted names -- Need to figure out how to pass this as a list
            #-- ['Rev.','Sr.','Sr',' sr','Jr.','Jr','jr','III','II','-DEL']
            a = row.replace('Rev.', '').replace('Sr.,', '').replace('Sr.', '').replace('Sr', '').replace(' sr', '').replace(
                'Jr.', '').replace('Jr', '').replace('jr', '').replace('III', '').replace('II', '').replace('-DEL', '')  # .replace(' , ','')
        ...
        # -- Pull out and name the appropriate columns
            ...
            fixed = pd.DataFrame(names, columns=['deathcertificate.about.firstName',
                                                 'deathcertificate.about.middleName', 'deathcertificate.about.lastName', 'deathcertificate.about.aka'])

        # -- Check if pre-formatted output file exists, if it does, this will make a sequential filename like "filename#" where # is the sequence
